...
class TestRAGRouting:
    ...
    def test_high_similarity_routes_to_rag(
        self, mock_embed, mock_search, mock_exists, mock_count, mock_avail, router
    ):
...
class TestEmptyKnowledgeBase:
    ...
    @patch("backend.agent.router.collection_exists", return_value=True)
    def test_empty_collection_routes_to_tool(self, mock_exists, mock_count, mock_avail, router):
    ...
    def test_no_collection_routes_to_tool(self, mock_exists, mock_avail, router):
        result = router.route("any question")
    ...
    @patch("backend.agent.router.is_available", return_value=False)
    def test_milvus_unavailable_routes_to_tool(self, mock_avail, router):
        result = router.route("any question")

        assert result["path"] == "tool"
        assert result["similarity_score"] == 0.0
        assert "empty" in result["reason"].lower() or "tool" in result["reason"].lower()
...
class TestRouterOutputValidation:
    ...
    @patch("backend.agent.router.get_document_count", return_value=0)
    @patch("backend.agent.router.collection_exists", return_value=True)
    def test_output_has_all_required_keys(self, mock_exists, mock_count, mock_avail, router):
        result = router.route("test")
